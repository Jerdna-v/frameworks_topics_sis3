...
router.post("/bulk", async (req, res) => {
  try {
    ...
    await DB.addCashFlowBulk(dt, Number(mid), uid, items);

    return res.json({ msg: "Cash flow saved." });
  } catch (err) {
    console.error("cashflow bulk error:", err);
...
router.post("/", async (req, res) => {
  try {
    ...
    await DB.addCashFlow(date, Number(mid), uid, cashtype, Number(amount), description || null);
