...
router.post("/", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!mid) return res.status(400).json({ success: false, msg: "Machine ID not found in file." });
    ...
    const rid = await DB.insertReport(uid, mid, reportDateTime);
