...
router.post("/", async (req, res) => {
  try {
    const { current, date, time, mid, description } = req.body || {};
    const uid = req.session?.user?.uid; // assuming session middleware sets this
    ...
    if (current) {
      ...
      if (isNaN(d.getTime())) return res.status(400).json({ msg: "Invalid date/time." });
      dateTime = d;
    }

    const pad2 = (n) => String(n).padStart(2, "0");
    const dt =
      `${dateTime.getFullYear()}-${pad2(dateTime.getMonth() + 1)}-${pad2(dateTime.getDate())} ` +
      `${pad2(dateTime.getHours())}:${pad2(dateTime.getMinutes())}:${pad2(dateTime.getSeconds())}`;

    await DB.addService(dt, Number(mid), uid, desc);

    return res.json({ msg: "Service entry saved." });
  } catch (err) {
...
router.get("/", async (req, res) => {
  try {
    ...
    const rows = await DB.listService(mid);
